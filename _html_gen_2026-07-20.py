#!/usr/bin/env python3
"""Generate analysis-report-2026-07-20.html from the markdown report."""
...
OUTDIR = Path('/sessions/gifted-blissful-thompson/mnt/local_754ac9df-27cd-4e02-8ee2-7d29ecd49486--outputs')
SRC_MD = OUTDIR / 'analysis-report-2026-07-20.md'
DST_HTML = OUTDIR / 'analysis-report-2026-07-20.html'
TEMPLATE_HTML = OUTDIR / 'analysis-report-2026-07-13.html'
...
SECTION_MAP = [
    ...
    ("Data Quality / Gaps", "gaps", "06"),
    ("Recommendations", "recommendations", "07"),
...
def normalize_md_links(html_body):
    def fix(m):
        ...
        if href.startswith('#'): return m.group(0)
...
def add_section_anchors(html_body):
    ...
    if not h2_positions: return html_body
    ...
    for idx, (start, end, title_html) in enumerate(h2_positions):
        ...
        out.append(inner); out.append('</section>')
    return ''.join(out)


def insert_citation_anchor(html_body):
    return re.sub(r'<section><h2>Citation Reference Table</h2>',
...
<div class="subtitle">Extraction 19 &nbsp;&nbsp;July 13 &ndash; July 20, 2026<br>Analysis executed 2026-07-20</div>
...
def main():
    ...
    stats = {
        "items": "61",
        "dominant": "40% SN+CN",
        "top_cluster": "Incidents / Failures",
        "incidents": "8",
        "patterns": "11",
        "batches": "9/9",
    }
    ...
    footer = '''<footer>
<p>AI Dev Sentiment Analysis &mdash; Extraction 19 &mdash; Generated 2026-07-20</p>
<p>Analysis Engine v1.17 &middot; Domain Config v1.2 &middot; Bootloader v1.9 &middot; HTML Engine v1.2</p>
</footer>'''
    head = re.sub(r'<meta name="description" content="[^"]*">',
        '<meta name="description" content="AI Dev Sentiment Analysis &mdash; July 13 to July 20, 2026 (E19, n=61 via cross-LLM escalation). MCP-mediated coding-agent RCE crystallizes as enterprise threat class; autonomous AI attackers hit Hugging Face and Nacos in production; verification-gap reaches quantitative consensus. Three NEW signals: consent-surface-erosion, agentic-threat-actor, chinese-open-weight-parity.">', head)
    head = re.sub(r'<meta property="og:title" content="[^"]*">',
        '<meta property="og:title" content="AI Dev Sentiment Analysis &mdash; E19 (July 13 &ndash; July 20, 2026)">', head)
    head = re.sub(r'<meta property="og:description" content="[^"]*">',
        '<meta property="og:description" content="MCP crystallizes as enterprise threat class; autonomous AI attackers hit Hugging Face and Nacos; verification-gap consensus + Microsoft 24% counter-signal; three NEW signals; Fable 5 credits-only Pro switch lands with r/ClaudeAI reaction cluster.">', head)
    head = re.sub(r'<title>.*?</title>',
        '<title>AI Dev Sentiment Analysis &mdash; E19 (July 13 &ndash; July 20, 2026)</title>', head)
    html = f'''{head}
<body>
{nav}
<div class="wrapper">
